...
<h1>
<?php
	esc_html_e( get_admin_page_title(), 'bkash-pgw-for-woocommerce' );
?>
	</h1>
<br>
	...
	<table id="refund-table" aria-describedby="refund table">
		<tr>
			...
			<td>
				...
				?>
				<input name="trxid" type="text" id="trxid" placeholder="Transaction ID" class="form-text-input" value="<?php esc_attr_e( $current_trx_id, 'bkash-pgw-for-woocommerce' ); ?> "/>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<input name="amount" type="text" id="amount" placeholder="Amount" class="form-text-input" value="<?php esc_attr_e( $amount ?? '', 'bkash-pgw-for-woocommerce' ); ?>"/>
			</td>
		</tr>
		<tr>
			<td>
...
<form action="#" method="post">
	...
	<table id="refund-status-table" aria-describedby="Refund Status Table">
		<tr>
			...
			<td>
				<input name="trxid" type="text" id="trxid" placeholder="Transaction ID" class="form-text-input" value="<?php esc_html_e( $current_trx_id, 'bkash-pgw-for-woocommerce' ); ?> "/>
			</td>
		</tr>
	</table>
...
if ( isset( $trx ) && is_string( $trx ) && ! empty( $trx ) ) {
	...
	<div id="message" class="bKash-hero-div woocommerce-message bKash-error">
		<p>
		<?php
			esc_html_e( $trx, 'bkash-pgw-for-woocommerce' );
		?>
			</p>
	</div>
	<?php
	...
	<div class="gateway-banner bKash-hero-div bKash-success">
		...
		<p class="main">
			<strong>
				Transaction ID:
				<?php
				esc_html_e( $trx['originalTrxID'] ?? '', 'bkash-pgw-for-woocommerce' );
				?>
			</strong>
		</p>
		<hr>
		<p>
			...
			<b>
			<?php
				esc_html_e( $trx['refundTrxID'] ?? '', 'bkash-pgw-for-woocommerce' );
			?>
				</b>
		</p>
		<p>Amount:
		...
		<ul>
			<li>Charge: <strong>
			<?php
					esc_html_e( $trx['charge'] ?? '', 'bkash-pgw-for-woocommerce' );
			?>
					</strong></li>
			<li>
				Completed At:
				<strong>
				<?php
					esc_html_e( $trx['completedTime'] ?? '', 'bkash-pgw-for-woocommerce' );
				?>
					</strong>
			</li>
		</ul>
		<p>
			...
			<?php
			esc_attr_e( $btn_class, 'bkash-pgw-for-woocommerce' )
			?>
			">
				Refund Status -
				<?php
				esc_html_e( $trx['transactionStatus'] ?? '', 'bkash-pgw-for-woocommerce' );
				?>
			</button>
		</p>
	</div>
