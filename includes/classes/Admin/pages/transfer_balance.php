...
<form action="#" method="post">
	...
	<table id="transfer-balance-table" aria-describedby="transfer balance">
		<tr>
			...
			<td>
				<input name="amount" type="text" id="amount" placeholder="Amount" class="form-text-input" value="<?php esc_html_e( $amount ?? '', 'bkash-pgw-for-woocommerce' ); ?>"/>
			</td>
		</tr>
		<tr>
			<td>
...
if ( isset( $trx ) && is_string( $trx ) && ! empty( $trx ) ) {
	...
	<div class="gateway-banner bKash-hero-div bKash-success">
		...
		<p>Transfer Type: <b>
		<?php
				esc_html_e( $trx['transferType'] ?? '', 'bkash-pgw-for-woocommerce' );
		?>
				</b></p>
		<p>Amount: <b>
		<?php
				esc_html_e( $trx['amount'] ?? '', 'bkash-pgw-for-woocommerce' );
		?>
				</b></p>
		<hr>
		<ul>
