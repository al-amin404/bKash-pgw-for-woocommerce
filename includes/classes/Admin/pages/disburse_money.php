...
<form action="#" method="post">
	...
	<table id="disburse-money-table" aria-describedby="disburse money">
		...
		<tr>
			...
			<td>
				<input name="receiver" type="tel" id="receiver" placeholder="Mobile number" class="form-text-input" value="<?php esc_attr_e( $receiver ?? '', 'bkash-pgw-for-woocommerce' ); ?>" pattern="^(?:\+88|01)?\d{11}$"/>
			</td>
		</tr>
		<tr>
			<td>
...
if ( isset( $trx ) && is_string( $trx ) && ! empty( $trx ) ) {
	...
	<div class="gateway-banner bKash-hero-div bKash-success">
		...
		<p>Disbursed To (bKash Customer Account):
			<b>
			<?php
			esc_html_e( $trx['receiverMSISDN'] ?? '', 'bkash-pgw-for-woocommerce' );
			?>
			</p>
		<p>Amount: <b>
		<?php
				esc_html_e( $trx['currency'] ?? '', 'bkash-pgw-for-woocommerce' );
		?>
				</b></p>
		<hr>
		<ul>
			<li>Invoice Number:
				<strong>
				<?php
					esc_html_e( $trx['merchantInvoiceNumber'] ?? '', 'bkash-pgw-for-woocommerce' );
				?>
					</strong>
			</li>
			<li>
