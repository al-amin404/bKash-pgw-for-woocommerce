...
use bKash\PGW\Blocks\BkashPaymentMethodType;
...
class WooCommerceBkashPgw {
	 ...
	 * @var    string
	 */
	public $name = 'bKash Payment Gateway for WooCommerce';
	...
	private function __construct() {
		$this->migrateLegacyGatewaySettings();
	...
	/**
	 * Copy WooCommerce gateway settings stored under the previous plugin slug.
	 *
	 * @return void
	 */
	private function migrateLegacyGatewaySettings() {
		$current_option = 'woocommerce_' . BKASH_FW_PLUGIN_SLUG . '_settings';
		$legacy_option  = 'woocommerce_' . BKASH_FW_LEGACY_PLUGIN_SLUG . '_settings';

		if ( false !== get_option( $current_option, false ) ) {
			return;
		}

		$legacy_settings = get_option( $legacy_option, false );
		if ( false !== $legacy_settings ) {
			add_option( $current_option, $legacy_settings );
		}
	}
	...
	final public function loadPluginTextDomain() {
		...
		$locale = apply_filters( 'plugin_locale', get_locale(), 'bkash-pgw-for-woocommerce' );
		$moFile = sprintf( '%1$s-%2$s.mo', 'bkash-pgw-for-woocommerce', $locale );

		// Setup paths to current locale file
		$moFileLocal  = $lang_dir . $moFile;
		$moFileGlobal = WP_LANG_DIR . '/' . 'bkash-pgw-for-woocommerce' . '/' . $moFile;

		if ( file_exists( $moFileGlobal ) ) {
			// Look in global /wp-content/languages/plugin-name/ folder
			load_textdomain( 'bkash-pgw-for-woocommerce', $moFileGlobal );
		} elseif ( file_exists( $moFileLocal ) ) {
			// Look in local /wp-content/plugins/plugin-name/languages/ folder
			load_textdomain( 'bkash-pgw-for-woocommerce', $moFileLocal );
		} else {
			// Load the default language files
			load_plugin_textdomain( 'bkash-pgw-for-woocommerce', false, $lang_dir );
		}
	}
	...
	/**
	 * Register Checkout Block payment method integration.
	 *
	 * @return void
	 */
	final public function registerBlocksSupport() {
		if ( ! class_exists( PaymentMethodRegistry::class ) || ! class_exists( BkashPaymentMethodType::class ) ) {
			return;
		}

		add_action(
			'woocommerce_blocks_payment_method_type_registration',
			array( $this, 'registerBkashBlocksPaymentMethod' )
		);
	}

	/**
	 * Register bKash method in WooCommerce Blocks payment registry.
	 *
	 * @param PaymentMethodRegistry $payment_method_registry Payment method registry.
	 *
	 * @return void
	 */
	final public function registerBkashBlocksPaymentMethod( PaymentMethodRegistry $payment_method_registry ) {
		$payment_method_registry->register( new BkashPaymentMethodType() );
	}
